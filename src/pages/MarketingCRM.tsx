  ...
  const fetchMetaPages = async () => {
    ...
    try {
      ...
      if (!res.ok || data?.ok === false) {
        ...
        setPageError(
          data?.error ||
          "Não foi possível listar suas páginas. Verifique se sua conta é ADMIN de uma Página no Facebook e se o app está em modo LIVE."
        );
        return;
      }
      ...
      if (!pages.length) {
        setPageError(
          "Nenhuma Página retornou para sua conta. Você precisa ser ADMIN de uma Página no Facebook (e, para Instagram, o IG precisa estar vinculado a essa Página)."
        );
      }
    } catch (e) {
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-white p-1 rounded-xl border border-gray-200 shadow-sm">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'funnel'
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeTab === 'creative'
          ...
          <button
            ...
            className="px-6 py-3 bg-gray-900 text-white rounded-xl font-bold text-sm hover:bg-gray-800 flex items-center gap-2 shadow-xl active:scale-95"
          >
            <Plus size={18} /> Lead
          </button>
        </div>
      ...
      {activeTab === 'funnel' && (
        <>
          <div className="mb-6 flex-shrink-0">
            {checkingAuth ? (
              <div className="h-24 bg-gray-100 rounded-3xl animate-pulse"></div>
            ) : !isConnected ? (
              <div className="bg-white p-6 rounded-3xl border border-blue-100 shadow-sm flex items-center justify-between animate-in zoom-in">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 bg-[#1877F2] text-white rounded-xl font-bold shadow-lg hover:bg-[#166fe5] active:scale-95"
                >
              ...
              <div className="flex flex-col gap-4 animate-in slide-in-from-top-4">
                <div className="flex justify-between items-start gap-4">
                  <div className="grid grid-cols-3 gap-4 flex-1">
                    <div className="bg-white p-4 rounded-2xl border border-blue-100 shadow-sm">
                      <span className="text-[10px] uppercase text-gray-400 font-black">Investimento</span>
                      <div className="text-2xl font-black text-gray-800">R$ {adsStats.spend.toFixed(2)}</div>
                    </div>
                    <div className="bg-white p-4 rounded-2xl border border-purple-100 shadow-sm">
                      <span className="text-[10px] uppercase text-gray-400 font-black">Cliques</span>
                      <div className="text-2xl font-black text-gray-800">{adsStats.clicks}</div>
                    </div>
                    <div
                      className="bg-green-500 text-white p-4 rounded-2xl shadow-lg flex items-center justify-center gap-2 font-bold cursor-pointer hover:bg-green-600 active:scale-95 transition-all"
                ...
                <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm flex flex-col gap-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 font-medium">
                          Sem Página selecionada, não dá pra puxar posts do Facebook nem mídia do Instagram Business.
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col md:flex-row gap-3">
                    ...
                    >
                      <option value="">
                        {metaPages.length ? '-- Selecione uma Página --' : (pagesLoading ? 'Carregando...' : 'Nenhuma página disponível')}
                      </option>
                      {metaPages.map((p) => (
                  ...
                  <div className="text-xs font-bold">
                    Status:{' '}
                    {pageSelectedOk ? (
                      <span className="text-green-600">Página selecionada ✅</span>
                    ) : (
                      <span className="text-gray-500">Seleção pendente</span>
                    )}
                  </div>
                </div>
              </div>
                ...
                {Object.entries(COLUMNS).map(([id, cfg]) => (
                  <div key={id} className="flex flex-col bg-gray-200/30 rounded-[2rem] w-80 h-full border border-gray-200/50">
                    <div className={`bg-white p-4 rounded-t-[2rem] border-t-8 ${cfg.color} font-black text-xs uppercase ${cfg.headerColor} flex justify-between`}>
                      <span>{cfg.title}</span>
                      <span className="bg-gray-100 px-2 rounded text-gray-500 text-[10px]">{getColumnLeads(id).length}</span>
                    </div>

                    <Droppable droppableId={id}>
                      {(p) => (
                        <div {...p.droppableProps} ref={p.innerRef} className="flex-1 p-4 flex flex-col gap-3 overflow-y-auto">
                          {getColumnLeads(id).map((item, idx) => (
                            <Draggable key={item.id} draggableId={String(item.id)} index={idx}>
                              {(p) => (
                                <div
                                  ...
                                  {...p.draggableProps}
                                  {...p.dragHandleProps}
                                  className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 group hover:border-blue-300 transition-all"
                                >
                                  <div className="font-bold text-sm text-gray-800">{item.name}</div>
                                  <div className="flex items-center gap-1 text-[10px] text-gray-400 mt-2 uppercase font-bold">
                                    <div className={`w-1.5 h-1.5 rounded-full ${item.source === 'Instagram'
                                      ? 'bg-purple-500'
          ...
          <div className="w-2/3 bg-white rounded-[2rem] p-6 shadow-sm border border-gray-200 overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex bg-gray-100 p-1 rounded-lg">
                <button
                  onClick={() => fetchMedia('instagram')}
                  className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${mediaSource === 'instagram'
                ...
                <button
                  onClick={() => fetchMedia('facebook')}
                  className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${mediaSource === 'facebook'
            ...
            {!isConnected ? (
              ...
              <div className="bg-yellow-50 p-6 rounded-2xl border border-yellow-100 flex flex-col items-center text-center">
                ...
                <p className="text-yellow-800 text-sm mb-4 max-w-md">
                  Vá na aba <b>Funil</b> e selecione uma Página do Facebook. Sem isso, não dá pra buscar posts do Facebook nem mídia do Instagram Business.
                </p>
              </div>
            ) : mediaLoading ? (
              ...
              <div className="bg-red-50 p-6 rounded-2xl border border-red-100 flex flex-col items-center text-center">
                ...
                <h4 className="text-red-800 font-bold mb-1">Atenção</h4>
              ...
              <div className="grid grid-cols-3 gap-4">
                {mediaList.map((media) => (
                  <div
                    key={media.id}
                    onClick={() => { setSelectedMedia(media); setAiCaption(''); }}
                    className={`relative aspect-square rounded-xl overflow-hidden cursor-pointer border-4 transition-all ${selectedMedia?.id === media.id
          ...
          <div className="w-1/3 flex flex-col gap-4">
            <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-200 flex-1 flex flex-col">
              <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                <Wand2 className="text-purple-600" /> Otimizador IA
              </h3>
              ...
              {selectedMedia ? (
                <>
                  ...
                  <div className="mt-6 flex-1 flex flex-col">
                    ...
                    />
                    <button
                      className="mt-2 text-purple-600 text-xs font-bold flex items-center gap-1 self-end hover:bg-purple-50 p-2 rounded-lg"
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 flex items-center justify-center backdrop-blur-sm p-4">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl w-full max-w-md animate-in fade-in zoom-in duration-200 border border-white">
            ...
            <form onSubmit={handleCreateLead} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-sm uppercase tracking-widest mt-4 shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all"
              >
                Salvar Lead
              </button>
            </form>
